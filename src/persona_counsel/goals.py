...
def load_goals(month: str, vault_root: Path | None = None) -> str:
...
def load_weekly_goals(week: str, vault_root: Path | None = None) -> str:
...
def load_annual_goals(year: str, vault_root: Path | None = None) -> str:
...
def goals_output_path(month: str, vault_root: Path | None = None) -> Path:
    root = vault_root or find_vault_root()
    year = month[:4]
    return root / "Goals" / year / "_monthly" / "reviews" / f"{month}-council.md"


def weekly_output_path(week: str, vault_root: Path | None = None) -> Path:
    root = vault_root or find_vault_root()
    year = week[:4]
    return root / "Goals" / year / "_weekly" / "reviews" / f"{week}-council.md"


def annual_output_path(year: str, vault_root: Path | None = None) -> Path:
    root = vault_root or find_vault_root()
...
def current_month() -> str:
    ...
    return datetime.now().astimezone().date().strftime("%Y-%m")
...
def current_year() -> str:
    ...
    return str(datetime.now().astimezone().date().year)
...
def load_council_report(period: str, vault_root: Path | None = None) -> str:
